...
pigeon_gyro::pigeon_gyro(ctre::phoenix::motorcontrol::can::TalonSRX& talon)
    : m_can_id(0), m_angle(0), m_rate(0), m_simDevice("Gyro:CTRE_pigeon", 0) {
  if (m_simDevice) {
//  m_simAngle =
//      m_simDevice.CreateDouble("angle_x", hal::SimDevice::kInput, 0.0);
//  m_simRate = m_simDevice.CreateDouble("rate_x", hal::SimDevice::kInput, 0.0);
  ...
  if (!m_simDevice) {
      _pidgey = new PigeonIMU(talon); /* Pigeon is on talon */
      _pidgey->ConfigFactoryDefault();
